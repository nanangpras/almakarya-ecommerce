                        ...
                        <div class="form-wrap">
                            <form action="{{ route('banner.update',$item->id)}}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label mb-10">Bagian</label>
                                            <select name="for" class="form-control @error('for') is invalid @enderror" tabindex="1">
                                                <option value="">--Pilih--</option>
                                                @foreach ($status as $id => $isi)
                                                    <option value="{{ $id}}" {{$item->for == $id ? 'selected' : ''}}>{{$isi}}</option>
                                                @endforeach
                                            </select>
                                            @error('for')
                                                    <span class="help-block"> {{$message}} </span>
                                            ...
                                            <div class="radio-list">
                                                ...
                                                <div class="radio-inline">
                                                    <div class="radio radio-success ">
                                                        <input type="radio" 
                                                            class="form-check-input @error('status') is invalid @enderror"
                                                            name="status" value="0" {{$item->status == 0 ? 'checked':''}}>
